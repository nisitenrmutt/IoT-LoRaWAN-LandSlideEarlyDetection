// ========= Sender Node: LoRa + Sensors (payload ย่อ + packet count ผ่าน deep sleep) =========
...
// ---------- LoRa CONFIG (ต้องตรงกับ MasterNode) ----------
long LORA_FREQ      = 920E6;   // 920 MHz (ให้ตรง Master)
int  LORA_SF        = 7;
long LORA_BW        = 125E3;
int  LORA_CR        = 5;
...
const float    SEND_INTERVAL_MINUTES = 0.1;        // 0.1 = 6 วินาที (ทดสอบ)
const uint32_t SEND_INTERVAL_SECONDS = SEND_INTERVAL_MINUTES * 60;
const bool     USE_DEEP_SLEEP        = true;

// ========= Packet counter อยู่ใน RTC memory =========
RTC_DATA_ATTR int counter = 0;  // จะไม่ reset เมื่อ deep sleep / ตื่นมาใหม่

// ชื่อ Node
const char *deviceName = "Node2";

// แสดง LoRa config ทาง Serial
...
void loop() {
  ...
  // ====== ส่ง LoRa: JSON แบบย่อ (ไม่มี space, key สั้น) ======
  ...
  // เพิ่ม counter แล้วค่อยเข้า deep sleep
  counter++;
