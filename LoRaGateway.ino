...
const char *ssid = "######";
const char *password = "##########";
const char *scriptUrl = "URL Google sheet script";
const char *lineNotifyToken = "##########";

bool wifiConnected = false;
String deviceName;     // String for Node Name
...
int cumulativeLostPackets = 0;      //
unsigned long lastPacketMillis = 0;
bool hasLastPacket = false;
bool linkTimeoutShown = false;
const unsigned long PACKET_TIMEOUT_MS = 300000UL; // 5 minute
...
void loop() {
  ...
  if (packetSize) {
    ...
    while (LoRa.available()) {
      ...
      // ====== Read JSON from Sender (payload) ======
      deviceName = doc["dn"].as<String>();
      int packetCount = doc["c"];    // 
  ...
  // If connection loss packet -> show last RSSI + last count
